...
EXPERT_CRITICAL_LITERATURE_REVIE
    ...
    Loads: reviewer_agent (evidence + writing), expert-figure-construction-v22 (figures).
```
...
| 2 | EXPERT | ×4 batches | `evidence-gathering-protocol` | `gate_evidence_compliance.json` | papers≥target, fulltext≥50%, conflicts>0, figure_data≥2/cluster |
| 3 | DATAML | single | `dataml-phases` | `gate_citation_infrastructure.json` | citation_key_map VID not null, ≥95% DOIs mapped |
| 4 | EXPERT | single | `scaffold-protocol` | `gate_scaffold_approved.json` | ≥2 figs/section, cross-refs between non-adjacent sections |
| 5 | DATAML | single | `dataml-phases` | `gate_evidence_curated.json` | coverage≥75%, cite_keys assigned, conflicts normalized |
| 6 | EXPERT | ×4 batches | `figure-audit-protocol` | `gate_figure_audit.json` | 0 REDESIGN remaining |
| 7 | EXPERT | ×4 batches | `section-writing-protocol` | `gate_sections_drafted.json` | cites/para≥4.0, `:::{dropdown}` count = `:::{figure}` count |
| 8 | EXPERT | ×4 batches | `critic-protocol` | `gate_critic_complete.json` | MUST_FIX=0 after send-back |
| 9 | DATAML | single | `dataml-phases` | `gate_bibliography.json` | entries=unique cite_keys, 0 Bhatt hits |
| 10 | EXPERT | single | `integration-protocol` | `gate_integration.json` | 6 passes documented, diffs in transitions |
| 11 | EXPERT | single | `integration-protocol` | `gate_intro_conclusion.json` | no new citations, abstract written |
| 12 | DATAML | single | `dataml-phases` | `gate_methods.json` | 8 subsections present |
| 13 | DATAML | single | `dataml-phases` | `gate_assembly.json` | 0 broken refs, toc=file count, plugins deployed |
| 14 | DATAML | single | `dataml-phases` | — | triples extracted into batches of 18 |
| 15 | EXPERT | ×4 batches | `verification-protocol` | — | ALL triples verified, no sampling |
| 16 | DATAML | single | `dataml-phases` | — | fix requests prepared |
| 17 | EXPERT | ×4 batches | `fix-protocol` | — | fixes executed with DB-verified replacements |
| 18 | DATAML | single | `dataml-phases` | — | fixes applied, integrity verified |
| 19 | DATAML | single | `dataml-phases` | `gate_repository_push.json` | myst build passes, toc complete |

### Delegation Pattern

For EXPERT phases: `"Load skill [sub-skill-name] and execute Phase N. Also load reviewer_agent. Your inputs: [artifact references]."`

For DATAML phases: `"Load skill dataml-phases and execute Phase N. Your inputs: [artifact references]."` Or use `send_message` to resume an existing DATAML child.
